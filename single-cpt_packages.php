...
$contact_btn_text = get_post_meta( get_the_ID(), '_package_contact_btn_text', true );

// Convert features textarea to array
$features_array = array_filter( array_map( 'trim', explode( "\n", $package_features ) ) );

// Generate button link - always use the main WhatsApp number
$contact_link = 'https://wa.me/994554401020?text=' . urlencode( 'Hi, I am interested in the ' . get_the_title() . ' package' );
?>
<!DOCTYPE html>
						...
						<div class="package-content">
							...
							<?php if ( ! empty( $contact_link ) ) : ?>
								<a href="<?php echo esc_url( $contact_link ); ?>" class="btn-red" target="_blank" rel="noopener">
									<?php
$pkg_single_default = function_exists( 'pll__' ) ? pll__( 'GET IN TOUCH' ) : __( 'GET IN TOUCH', 'yacht-rental' );
echo esc_html( ! empty( $contact_btn_text ) ? $contact_btn_text : $pkg_single_default );
?>
								</a>
							<?php endif; ?>
						</div>
				...
				// Show FAQ section
				if ( ! empty( $faqs ) ) :
					$faq_title = get_post_meta( get_the_ID(), '_package_faq_title', true );
					if ( empty( $faq_title ) ) {
						$faq_title = __( 'Frequently Asked Questions', 'yacht-rental' );
					}
				?>
