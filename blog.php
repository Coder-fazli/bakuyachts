...
if ( function_exists( 'yacht_rental_elementor_is_preview' ) && yacht_rental_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			yacht_rental_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	yacht_rental_new_main_query(
		array(
			'post_type'      => yacht_rental_get_theme_option( 'post_type' ),
			'category'       => yacht_rental_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'yacht_rental_filter_get_template_part', yacht_rental_blog_archive_get_template() ) );
}
