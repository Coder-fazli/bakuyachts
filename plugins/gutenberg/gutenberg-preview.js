...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			yacht_rental_create_observer( 'yacht-rental-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				yacht_rental_gutenberg_editor_init();
			} );
		} else {
		 	yacht_rental_gutenberg_editor_init();
		}
	...
	function yacht_rental_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor.$frame.length ) {
					if ( editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'yacht_rental_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'yacht_rental_inited' ) ) {
	...
	// Init on page load
	function yacht_rental_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = yacht_rental_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + YACHT_RENTAL_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + YACHT_RENTAL_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in YACHT_RENTAL_STORAGE['override_classes'] ) {
				$obj.addClass( YACHT_RENTAL_STORAGE['override_classes'][i].replace( '%s', YACHT_RENTAL_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( yacht_rental_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && YACHT_RENTAL_STORAGE['sidebar_position'] != 'hide' ) {
				if ( yacht_rental_apply_filters( 'yacht_rental_filter_wrap_gutenberg_editor_with_sidebar', false ) ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'yacht_rental_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			if ( yacht_rental_apply_filters( 'yacht_rental_filter_add_scheme_to_widgets_editor', true ) ) {
				...
				yacht_rental_remove_observer( 'yacht-rental-check-editor-styles-wrapper' );
				yacht_rental_create_observer( 'yacht-rental-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'yacht_rental_inited' );
	}
} );
