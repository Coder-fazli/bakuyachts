...
jQuery( document ).ready(
	function() {
		...
		yacht_rental_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="yacht_rental_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.yacht_rental_post_editor' );
						if ( link.length > 0 ) {
							yacht_rental_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.yacht_rental_post_editor', function(e) {
						yacht_rental_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('yacht_rental_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function yacht_rental_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
