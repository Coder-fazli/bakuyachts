...
function yr_register_yacht_category_taxonomy() {
	...
	$args = array(
		...
		'show_tagcloud'              => true,
		'show_in_rest'               => true,
		'rewrite'                    => array( 'slug' => 'yachts', 'with_front' => false, 'hierarchical' => true ),
	);
...
function yr_yachts_check_flush() {
	...
	if ( version_compare( $version, '1.4', '<' ) ) {
		...
		update_option( 'yr_yachts_version', '1.4' );
	}
...
// Force enable Yachts CPT in Polylang settings
function yr_yachts_polylang_enable_cpt() {
	...
	if ( $options && is_array( $options ) ) {
		...
		if ( ! in_array( 'cpt_yachts', $options['post_types'] ) ) {
			$options['post_types'][] = 'cpt_yachts';
			update_option( 'polylang', $options );
		}
	}
}
add_action( 'admin_init', 'yr_yachts_polylang_enable_cpt' );

// Ensure Yachts archive filters by current language
function yr_yachts_archive_filter_language( $query ) {
	if ( is_admin() || ! $query->is_main_query() ) {
		return;
	}

	// Only apply to yachts archive
...
add_action( 'pre_get_posts', 'yr_yachts_archive_filter_language' );
...
function yr_yacht_meta_box_callback( $post ) {
	...
	$price_label = get_post_meta( $post->ID, '_yr_yacht_price_label_text', true );
	$length_label = get_post_meta( $post->ID, '_yr_yacht_length_label', true );
	$cabins_label = get_post_meta( $post->ID, '_yr_yacht_cabins_label', true );
	$guests_label = get_post_meta( $post->ID, '_yr_yacht_guests_label', true );
	?>
	<style>
		.yr-field-group { margin-bottom: 20px; padding: 15px; background: #f9f9f9; border: 1px solid #e0e0e0; border-radius: 4px; }
		.yr-field-group label { font-weight: 600; display: block; margin-bottom: 5px; }
		.yr-field-row { display: flex; gap: 15px; align-items: flex-start; }
		.yr-field-col { flex: 1; }
		.yr-field-col input { width: 100%; }
		.yr-field-col .description { margin-top: 4px; font-size: 12px; color: #666; }
	</style>

	<div class="yr-field-group">
		<label><?php _e( 'Price', 'yacht-rental' ); ?></label>
		<div class="yr-field-row">
			<div class="yr-field-col">
				<input type="text" name="yr_yacht_price_label_text" value="<?php echo esc_attr( $price_label ); ?>" class="regular-text" placeholder="<?php esc_attr_e( 'Price (per hour)', 'yacht-rental' ); ?>" />
				<p class="description"><?php _e( 'Label (default: Price (per hour))', 'yacht-rental' ); ?></p>
			</div>
			<div class="yr-field-col">
				<input type="text" name="yr_yacht_price" value="<?php echo esc_attr( $price ); ?>" class="regular-text" placeholder="AED 1,300/hr" />
				<p class="description"><?php _e( 'Value', 'yacht-rental' ); ?></p>
			</div>
		</div>
	</div>

	<div class="yr-field-group">
		<label><?php _e( 'Length', 'yacht-rental' ); ?></label>
		<div class="yr-field-row">
			<div class="yr-field-col">
				<input type="text" name="yr_yacht_length_label" value="<?php echo esc_attr( $length_label ); ?>" class="regular-text" placeholder="<?php esc_attr_e( 'Length (ft)', 'yacht-rental' ); ?>" />
				<p class="description"><?php _e( 'Label (default: Length (ft))', 'yacht-rental' ); ?></p>
			</div>
			<div class="yr-field-col">
				<input type="text" name="yr_yacht_length" value="<?php echo esc_attr( $length ); ?>" class="regular-text" placeholder="60 ft" />
				<p class="description"><?php _e( 'Value', 'yacht-rental' ); ?></p>
			</div>
		</div>
	</div>
	...
	<div class="yr-field-group">
		<label><?php _e( 'Cabins', 'yacht-rental' ); ?></label>
		<div class="yr-field-row">
			<div class="yr-field-col">
				<input type="text" name="yr_yacht_cabins_label" value="<?php echo esc_attr( $cabins_label ); ?>" class="regular-text" placeholder="<?php esc_attr_e( 'Cabins', 'yacht-rental' ); ?>" />
				<p class="description"><?php _e( 'Label (default: Cabins)', 'yacht-rental' ); ?></p>
			</div>
			<div class="yr-field-col">
				<input type="text" name="yr_yacht_cabins" value="<?php echo esc_attr( $cabins ); ?>" class="regular-text" placeholder="3" />
				<p class="description"><?php _e( 'Value', 'yacht-rental' ); ?></p>
			</div>
		</div>
	</div>
	...
	<div class="yr-field-group">
		<label><?php _e( 'Guests', 'yacht-rental' ); ?></label>
		<div class="yr-field-row">
			<div class="yr-field-col">
				<input type="text" name="yr_yacht_guests_label" value="<?php echo esc_attr( $guests_label ); ?>" class="regular-text" placeholder="<?php esc_attr_e( 'Max Guests', 'yacht-rental' ); ?>" />
				<p class="description"><?php _e( 'Label (default: Max Guests)', 'yacht-rental' ); ?></p>
			</div>
			<div class="yr-field-col">
				<input type="text" name="yr_yacht_guests" value="<?php echo esc_attr( $guests ); ?>" class="regular-text" placeholder="15" />
				<p class="description"><?php _e( 'Value', 'yacht-rental' ); ?></p>
			</div>
		</div>
	</div>
...
function yr_yacht_contact_callback( $post ) {
	...
	$whatsapp_btn_text = get_post_meta( $post->ID, '_yr_yacht_whatsapp_btn_text', true );
	$contact_btn_text = get_post_meta( $post->ID, '_yr_yacht_contact_btn_text', true );
	?>
	<table class="form-table">
		<tr>
			<th><label for="yr_yacht_whatsapp"><?php _e( 'WhatsApp Number', 'yacht-rental' ); ?></label></th>
			<td><input type="text" id="yr_yacht_whatsapp" name="yr_yacht_whatsapp" value="<?php echo esc_attr( $whatsapp ); ?>" class="regular-text" placeholder="+994554401020" /></td>
		</tr>
		<tr>
			<th><label for="yr_yacht_phone"><?php _e( 'Phone Number', 'yacht-rental' ); ?></label></th>
			<td><input type="text" id="yr_yacht_phone" name="yr_yacht_phone" value="<?php echo esc_attr( $phone ); ?>" class="regular-text" placeholder="+994554401020" /></td>
		</tr>
	</table>

	<h4 style="margin-top: 20px; padding-top: 15px; border-top: 1px solid #ddd;"><?php _e( 'Button Titles', 'yacht-rental' ); ?></h4>
	<p class="description" style="margin-bottom: 15px;"><?php _e( 'Customize the button text for different languages', 'yacht-rental' ); ?></p>
	<table class="form-table">
		<tr>
			<th><label for="yr_yacht_whatsapp_btn_text"><?php _e( 'WhatsApp Button Text', 'yacht-rental' ); ?></label></th>
			<td>
				<input type="text" id="yr_yacht_whatsapp_btn_text" name="yr_yacht_whatsapp_btn_text" value="<?php echo esc_attr( $whatsapp_btn_text ); ?>" class="regular-text" placeholder="WHATSAPP US" />
				<p class="description"><?php _e( 'Default: "WHATSAPP US"', 'yacht-rental' ); ?></p>
			</td>
		</tr>
		<tr>
			<th><label for="yr_yacht_contact_btn_text"><?php _e( 'Get In Touch Button Text', 'yacht-rental' ); ?></label></th>
			<td>
				<input type="text" id="yr_yacht_contact_btn_text" name="yr_yacht_contact_btn_text" value="<?php echo esc_attr( $contact_btn_text ); ?>" class="regular-text" placeholder="GET IN TOUCH" />
				<p class="description"><?php _e( 'Default: "GET IN TOUCH"', 'yacht-rental' ); ?></p>
			</td>
		</tr>
	</table>
	<?php
...
function yr_yacht_features_callback( $post ) {
	...
	$features_title = get_post_meta( $post->ID, '_yr_yacht_features_title', true );
	?>
	<div style="margin-bottom: 20px; padding: 15px; background: #f0f6fc; border: 1px solid #c5d9ed; border-radius: 4px;">
		<label style="font-weight: 600; display: block; margin-bottom: 5px;"><?php _e( 'Section Title', 'yacht-rental' ); ?></label>
		<input type="text" name="yr_yacht_features_title" value="<?php echo esc_attr( $features_title ); ?>" class="large-text" placeholder="<?php esc_attr_e( 'Key Features of [yacht name]', 'yacht-rental' ); ?>" />
		<p class="description"><?php _e( 'Leave empty to use default: "Key Features of [yacht name]"', 'yacht-rental' ); ?></p>
	</div>
...
function yr_yacht_offers_callback( $post ) {
	...
	$offers_title = get_post_meta( $post->ID, '_yr_yacht_offers_title', true );
	?>
	<div style="margin-bottom: 20px; padding: 15px; background: #f0f6fc; border: 1px solid #c5d9ed; border-radius: 4px;">
		<label style="font-weight: 600; display: block; margin-bottom: 5px;"><?php _e( 'Section Title', 'yacht-rental' ); ?></label>
		<input type="text" name="yr_yacht_offers_title" value="<?php echo esc_attr( $offers_title ); ?>" class="large-text" placeholder="<?php esc_attr_e( 'We also Offer:', 'yacht-rental' ); ?>" />
		<p class="description"><?php _e( 'Leave empty to use default: "We also Offer:"', 'yacht-rental' ); ?></p>
	</div>
...
function yr_yacht_faq_callback( $post ) {
	...
	$faq_title = get_post_meta( $post->ID, '_yr_yacht_faq_title', true );
	?>
	<div style="margin-bottom: 20px; padding: 15px; background: #f0f6fc; border: 1px solid #c5d9ed; border-radius: 4px;">
		<label style="font-weight: 600; display: block; margin-bottom: 5px;"><?php _e( 'Section Title', 'yacht-rental' ); ?></label>
		<input type="text" name="yr_yacht_faq_title" value="<?php echo esc_attr( $faq_title ); ?>" class="large-text" placeholder="<?php esc_attr_e( 'Frequently Asked Questions', 'yacht-rental' ); ?>" />
		<p class="description"><?php _e( 'Leave empty to use default: "Frequently Asked Questions"', 'yacht-rental' ); ?></p>
	</div>
...
function yr_save_yacht_meta( $post_id ) {
	...
	if ( isset( $_POST['yr_yacht_price_label_text'] ) ) {
		update_post_meta( $post_id, '_yr_yacht_price_label_text', sanitize_text_field( $_POST['yr_yacht_price_label_text'] ) );
	}
	if ( isset( $_POST['yr_yacht_length_label'] ) ) {
		update_post_meta( $post_id, '_yr_yacht_length_label', sanitize_text_field( $_POST['yr_yacht_length_label'] ) );
	}
	if ( isset( $_POST['yr_yacht_cabins_label'] ) ) {
		update_post_meta( $post_id, '_yr_yacht_cabins_label', sanitize_text_field( $_POST['yr_yacht_cabins_label'] ) );
	}
	if ( isset( $_POST['yr_yacht_guests_label'] ) ) {
		update_post_meta( $post_id, '_yr_yacht_guests_label', sanitize_text_field( $_POST['yr_yacht_guests_label'] ) );
	}
	...
	// Button Titles
	if ( isset( $_POST['yr_yacht_whatsapp_btn_text'] ) ) {
		update_post_meta( $post_id, '_yr_yacht_whatsapp_btn_text', sanitize_text_field( $_POST['yr_yacht_whatsapp_btn_text'] ) );
	}
	if ( isset( $_POST['yr_yacht_contact_btn_text'] ) ) {
		update_post_meta( $post_id, '_yr_yacht_contact_btn_text', sanitize_text_field( $_POST['yr_yacht_contact_btn_text'] ) );
	}
	...
	if ( isset( $_POST['yr_yacht_features_title'] ) ) {
		update_post_meta( $post_id, '_yr_yacht_features_title', sanitize_text_field( $_POST['yr_yacht_features_title'] ) );
	}
	...
	if ( isset( $_POST['yr_yacht_offers_title'] ) ) {
		update_post_meta( $post_id, '_yr_yacht_offers_title', sanitize_text_field( $_POST['yr_yacht_offers_title'] ) );
	}
	...
	if ( isset( $_POST['yr_yacht_faq_title'] ) ) {
		update_post_meta( $post_id, '_yr_yacht_faq_title', sanitize_text_field( $_POST['yr_yacht_faq_title'] ) );
	}
}
	...
	foreach ( $demo_yachts as $yacht_data ) {
		...
		if ( $post_id && ! is_wp_error( $post_id ) ) {
			...
			if ( $yacht_data['title'] === 'Sky 92ft "LUNA"' ) {
				...
				// Contact
				update_post_meta( $post_id, '_yr_yacht_whatsapp', '+994554401020' );
				update_post_meta( $post_id, '_yr_yacht_phone', '+994554401020' );
...
function yr_yacht_cards_shortcode( $atts ) {
	...
	.yr-yacht-grid .bky-yacht-card:hover {
		...
		<div class="yr-shortcode-container">
			<div class="yr-yacht-grid">
				...
				while ( $yachts->have_posts() ) : $yachts->the_post();
					...
					if ( empty( $whatsapp ) ) {
						$whatsapp = '+994554401020'; // Default WhatsApp
					}
					$whatsapp_clean = preg_replace( '/[^0-9]/', '', $whatsapp );

					// Custom Labels (with defaults)
					$cabins_label = get_post_meta( get_the_ID(), '_yr_yacht_cabins_label', true );
					$guests_label = get_post_meta( get_the_ID(), '_yr_yacht_guests_label', true );
					if ( empty( $cabins_label ) ) $cabins_label = __( 'cabins', 'yacht-rental' );
					if ( empty( $guests_label ) ) $guests_label = __( 'Guests', 'yacht-rental' );
					?>
					<div class="bky-yacht-card">
						...
						<div class="bky-yacht-content">
							...
							<div class="bky-yacht-features">
								...
								<?php if ( $cabins ) : ?>
									<div class="bky-feature-item">
										...
										<span><?php echo esc_html( $cabins . ' ' . $cabins_label ); ?></span>
									</div>
								<?php endif; ?>
								<?php if ( $guests ) : ?>
									<div class="bky-feature-item">
										...
										<span><?php echo esc_html( $guests . ' ' . $guests_label ); ?></span>
									</div>
								<?php endif; ?>
							</div>
							...
							<div class="bky-yacht-actions">
								<a href="https://wa.me/<?php echo esc_attr( $whatsapp_clean ); ?>?text=<?php echo urlencode( 'Hi, I am interested in ' . get_the_title() ); ?>"
								...
								<?php $yacht_btn_text = get_post_meta( get_the_ID(), '_yr_yacht_contact_btn_text', true ); ?>
								<a href="<?php the_permalink(); ?>" class="bky-yacht-btn bky-yacht-btn-view">
									<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="22" y1="2" x2="11" y2="13"></line><polygon points="22 2 15 22 11 13 2 9 22 2"></polygon></svg>
									<?php echo esc_html( ! empty( $yacht_btn_text ) ? $yacht_btn_text : __( 'GET IN TOUCH', 'yacht-rental' ) ); ?>
								</a>
							</div>
						</div>
					</div>
...
/**
 * Yacht Button Colors Admin Settings
 */

// Add submenu page under Yachts
function yr_button_colors_menu() {
	add_submenu_page(
		'edit.php?post_type=cpt_yachts',
		__( 'Button Colors', 'yacht-rental' ),
		__( 'Button Colors', 'yacht-rental' ),
		'manage_options',
		'yr-button-colors',
		'yr_button_colors_page'
	);
}
add_action( 'admin_menu', 'yr_button_colors_menu' );

// Register settings
function yr_button_colors_settings_init() {
	register_setting( 'yr_button_colors', 'yr_whatsapp_btn_bg', array(
		'type' => 'string',
		'default' => '#25D366',
		'sanitize_callback' => 'sanitize_hex_color',
	) );
	register_setting( 'yr_button_colors', 'yr_whatsapp_btn_text', array(
		'type' => 'string',
		'default' => '#ffffff',
		'sanitize_callback' => 'sanitize_hex_color',
	) );
	register_setting( 'yr_button_colors', 'yr_whatsapp_btn_hover_bg', array(
		'type' => 'string',
		'default' => '#128C7E',
		'sanitize_callback' => 'sanitize_hex_color',
	) );
	register_setting( 'yr_button_colors', 'yr_view_btn_bg', array(
		'type' => 'string',
		'default' => '#A62946',
		'sanitize_callback' => 'sanitize_hex_color',
	) );
	register_setting( 'yr_button_colors', 'yr_view_btn_text', array(
		'type' => 'string',
		'default' => '#ffffff',
		'sanitize_callback' => 'sanitize_hex_color',
	) );
	register_setting( 'yr_button_colors', 'yr_view_btn_hover_bg', array(
		'type' => 'string',
		'default' => '#8B1F3A',
		'sanitize_callback' => 'sanitize_hex_color',
	) );
}
add_action( 'admin_init', 'yr_button_colors_settings_init' );

// Enqueue color picker
function yr_button_colors_admin_scripts( $hook ) {
	if ( 'cpt_yachts_page_yr-button-colors' !== $hook ) {
		return;
	}
	wp_enqueue_style( 'wp-color-picker' );
	wp_enqueue_script( 'wp-color-picker' );
}
add_action( 'admin_enqueue_scripts', 'yr_button_colors_admin_scripts' );

// Settings page HTML
function yr_button_colors_page() {
	$whatsapp_bg = get_option( 'yr_whatsapp_btn_bg', '#25D366' );
	$whatsapp_text = get_option( 'yr_whatsapp_btn_text', '#ffffff' );
	$whatsapp_hover = get_option( 'yr_whatsapp_btn_hover_bg', '#128C7E' );
	$view_bg = get_option( 'yr_view_btn_bg', '#A62946' );
	$view_text = get_option( 'yr_view_btn_text', '#ffffff' );
	$view_hover = get_option( 'yr_view_btn_hover_bg', '#8B1F3A' );
	?>
	<div class="wrap">
		<h1><?php _e( 'Yacht Button Colors', 'yacht-rental' ); ?></h1>
		<p><?php _e( 'Customize the colors of WhatsApp and View Details buttons on yacht cards.', 'yacht-rental' ); ?></p>

		<form method="post" action="options.php">
			<?php settings_fields( 'yr_button_colors' ); ?>
			...
			<div class="yr-color-settings">
				<div class="yr-color-section">
					...
					<div class="yr-color-row">
						<label for="yr_whatsapp_btn_bg"><?php _e( 'Background Color', 'yacht-rental' ); ?></label>
						<input type="text" id="yr_whatsapp_btn_bg" name="yr_whatsapp_btn_bg" value="<?php echo esc_attr( $whatsapp_bg ); ?>" class="yr-color-picker" />
					</div>

					<div class="yr-color-row">
						<label for="yr_whatsapp_btn_text"><?php _e( 'Text Color', 'yacht-rental' ); ?></label>
						<input type="text" id="yr_whatsapp_btn_text" name="yr_whatsapp_btn_text" value="<?php echo esc_attr( $whatsapp_text ); ?>" class="yr-color-picker" />
					</div>

					<div class="yr-color-row">
						<label for="yr_whatsapp_btn_hover_bg"><?php _e( 'Hover Background Color', 'yacht-rental' ); ?></label>
						<input type="text" id="yr_whatsapp_btn_hover_bg" name="yr_whatsapp_btn_hover_bg" value="<?php echo esc_attr( $whatsapp_hover ); ?>" class="yr-color-picker" />
					</div>
				</div>
				...
				<div class="yr-color-section">
					...
					<div class="yr-color-row">
						<label for="yr_view_btn_bg"><?php _e( 'Background Color', 'yacht-rental' ); ?></label>
						<input type="text" id="yr_view_btn_bg" name="yr_view_btn_bg" value="<?php echo esc_attr( $view_bg ); ?>" class="yr-color-picker" />
					</div>

					<div class="yr-color-row">
						<label for="yr_view_btn_text"><?php _e( 'Text Color', 'yacht-rental' ); ?></label>
						<input type="text" id="yr_view_btn_text" name="yr_view_btn_text" value="<?php echo esc_attr( $view_text ); ?>" class="yr-color-picker" />
					</div>

					<div class="yr-color-row">
						<label for="yr_view_btn_hover_bg"><?php _e( 'Hover Background Color', 'yacht-rental' ); ?></label>
						<input type="text" id="yr_view_btn_hover_bg" name="yr_view_btn_hover_bg" value="<?php echo esc_attr( $view_hover ); ?>" class="yr-color-picker" />
					</div>
				</div>

				<div class="yr-preview">
					<h3><?php _e( 'Preview', 'yacht-rental' ); ?></h3>
					<button type="button" class="yr-preview-btn" id="yr-preview-whatsapp" style="background: <?php echo esc_attr( $whatsapp_bg ); ?>; color: <?php echo esc_attr( $whatsapp_text ); ?>;">
						<svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
						WHATSAPP
					</button>
					<button type="button" class="yr-preview-btn" id="yr-preview-view" style="background: <?php echo esc_attr( $view_bg ); ?>; color: <?php echo esc_attr( $view_text ); ?>;">
	...
	jQuery(document).ready(function($) {
		...
		function updatePreview() {
			setTimeout(function() {
				var waBg = $('#yr_whatsapp_btn_bg').val();
				var waText = $('#yr_whatsapp_btn_text').val();
				var viewBg = $('#yr_view_btn_bg').val();
				var viewText = $('#yr_view_btn_text').val();

				$('#yr-preview-whatsapp').css({ 'background': waBg, 'color': waText });
				$('#yr-preview-view').css({ 'background': viewBg, 'color': viewText });
			}, 100);
		}
	});
	</script>
...
// Output dynamic button styles on frontend
function yr_output_button_colors_css() {
	if ( is_admin() ) {
		return;
	}
	...
	?>
	<style id="yr-button-colors-css">
	/* Yacht Card Buttons - Dynamic Colors */
	.bky-yacht-btn-whatsapp {
...
add_action( 'wp_head', 'yr_output_button_colors_css', 100 );

/**
 * Yacht Archive SEO Settings (per language)
 */

function yr_yachts_archive_seo_menu() {
	add_submenu_page(
		'edit.php?post_type=cpt_yachts',
		__( 'Archive SEO', 'yacht-rental' ),
		__( 'Archive SEO', 'yacht-rental' ),
		'manage_options',
		'yr-yachts-archive-seo',
		'yr_yachts_archive_seo_page'
	);
}
add_action( 'admin_menu', 'yr_yachts_archive_seo_menu' );

function yr_yachts_archive_seo_page() {
	...
	if ( isset( $_POST['yr_yachts_seo_save'] ) && check_admin_referer( 'yr_yachts_seo_nonce' ) ) {
		foreach ( $languages as $lang_code => $lang_name ) {
			update_option( "yr_yachts_archive_meta_title_{$lang_code}", sanitize_text_field( $_POST["yr_yachts_meta_title_{$lang_code}"] ?? '' ) );
			update_option( "yr_yachts_archive_meta_desc_{$lang_code}", sanitize_textarea_field( $_POST["yr_yachts_meta_desc_{$lang_code}"] ?? '' ) );
		}
		echo '<div class="notice notice-success is-dismissible"><p>' . __( 'SEO settings saved.', 'yacht-rental' ) . '</p></div>';
	}
	?>
	<div class="wrap">
		<h1><?php _e( 'Yachts Archive — SEO Settings', 'yacht-rental' ); ?></h1>
		<p class="description"><?php _e( 'Set the meta title and meta description for the Yachts archive page. These override Rank Math for each language.', 'yacht-rental' ); ?></p>
		<form method="post">
			<?php wp_nonce_field( 'yr_yachts_seo_nonce' ); ?>
			<?php foreach ( $languages as $lang_code => $lang_name ) : ?>
				...
				<table class="form-table">
					<tr>
						...
						<td><input type="text" name="yr_yachts_meta_title_<?php echo esc_attr( $lang_code ); ?>" value="<?php echo esc_attr( get_option( "yr_yachts_archive_meta_title_{$lang_code}", '' ) ); ?>" class="large-text" placeholder="<?php esc_attr_e( 'Yacht Rental Baku | Luxury Yachts', 'yacht-rental' ); ?>" /></td>
					</tr>
					<tr>
						<th><label><?php _e( 'Meta Description', 'yacht-rental' ); ?></label></th>
						<td><textarea name="yr_yachts_meta_desc_<?php echo esc_attr( $lang_code ); ?>" class="large-text" rows="3" placeholder="<?php esc_attr_e( 'Browse our luxury yacht fleet in Baku...', 'yacht-rental' ); ?>"><?php echo esc_textarea( get_option( "yr_yachts_archive_meta_desc_{$lang_code}", '' ) ); ?></textarea></td>
					</tr>
				</table>
			<?php endforeach; ?>
			<p class="submit"><input type="submit" name="yr_yachts_seo_save" class="button button-primary" value="<?php _e( 'Save SEO Settings', 'yacht-rental' ); ?>" /></p>
		</form>
	</div>
	<?php
}
