...
if ( ! function_exists( 'yacht_rental_storage_unset' ) ) {
	/**
	 ...
	 */
	function yacht_rental_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $YACHT_RENTAL_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			if ( isset( $YACHT_RENTAL_STORAGE[ $var_name ][ $key ][ $key2 ] ) ) {
				unset( $YACHT_RENTAL_STORAGE[ $var_name ][ $key ][ $key2 ] );
			}
		} elseif ( '' !== $key ) {
			if ( isset( $YACHT_RENTAL_STORAGE[ $var_name ][ $key ] ) ) {
				unset( $YACHT_RENTAL_STORAGE[ $var_name ][ $key ] );
			}
		} else {
			if ( isset( $YACHT_RENTAL_STORAGE[ $var_name ] ) ) {
				unset( $YACHT_RENTAL_STORAGE[ $var_name ] );
			}
		}
	}
}
...
if ( ! function_exists( 'yacht_rental_storage_inc' ) ) {
	...
	function yacht_rental_storage_inc( $var_name, $value = 1 ) {
		global $YACHT_RENTAL_STORAGE;
		if ( empty( $YACHT_RENTAL_STORAGE[ $var_name ] ) ) {
			$YACHT_RENTAL_STORAGE[ $var_name ] = 0;
		}
		$YACHT_RENTAL_STORAGE[ $var_name ] += $value;
	}
}
...
if ( ! function_exists( 'yacht_rental_storage_get_array' ) ) {
	...
	function yacht_rental_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $YACHT_RENTAL_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $YACHT_RENTAL_STORAGE[ $var_name ][ $key ] ) ? $YACHT_RENTAL_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $YACHT_RENTAL_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $YACHT_RENTAL_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}
...
if ( ! function_exists( 'yacht_rental_storage_set_array' ) ) {
	...
	function yacht_rental_storage_set_array( $var_name, $key, $value ) {
		global $YACHT_RENTAL_STORAGE;
		if ( ! isset( $YACHT_RENTAL_STORAGE[ $var_name ] ) ) {
			$YACHT_RENTAL_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $YACHT_RENTAL_STORAGE[ $var_name ] ) ) {
			if ( '' === $key ) {
				$YACHT_RENTAL_STORAGE[ $var_name ][] = $value;
			} else {
...
if ( ! function_exists( 'yacht_rental_storage_set_array2' ) ) {
	...
	function yacht_rental_storage_set_array2( $var_name, $key, $key2, $value ) {
		...
		if ( ! isset( $YACHT_RENTAL_STORAGE[ $var_name ][ $key ] ) ) {
			$YACHT_RENTAL_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( is_array( $YACHT_RENTAL_STORAGE[ $var_name ][ $key ] ) ) {
			if ( '' === $key2 ) {
				$YACHT_RENTAL_STORAGE[ $var_name ][ $key ][] = $value;
			} else {
...
if ( ! function_exists( 'yacht_rental_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function yacht_rental_storage_merge_array( $var_name, $key, $value ) {
		global $YACHT_RENTAL_STORAGE;
		if ( ! isset( $YACHT_RENTAL_STORAGE[ $var_name ] ) ) {
			$YACHT_RENTAL_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$YACHT_RENTAL_STORAGE[ $var_name ] = array_merge( $YACHT_RENTAL_STORAGE[ $var_name ], $value );
		} else {
			$YACHT_RENTAL_STORAGE[ $var_name ][ $key ] = array_merge( $YACHT_RENTAL_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}
...
if ( ! function_exists( 'yacht_rental_storage_inc_array' ) ) {
	...
	function yacht_rental_storage_inc_array( $var_name, $key, $value = 1 ) {
		...
		if ( empty( $YACHT_RENTAL_STORAGE[ $var_name ][ $key ] ) ) {
			$YACHT_RENTAL_STORAGE[ $var_name ][ $key ] = 0;
		}
		$YACHT_RENTAL_STORAGE[ $var_name ][ $key ] += $value;
	}
}
...
if ( ! function_exists( 'yacht_rental_storage_concat_array' ) ) {
	...
	function yacht_rental_storage_concat_array( $var_name, $key, $value ) {
		...
		if ( empty( $YACHT_RENTAL_STORAGE[ $var_name ][ $key ] ) ) {
			$YACHT_RENTAL_STORAGE[ $var_name ][ $key ] = '';
		}
		$YACHT_RENTAL_STORAGE[ $var_name ][ $key ] .= $value;
	}
}
